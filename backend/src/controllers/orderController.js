...
async function submitOrder(req, res) {
  ...
  //    The client gets their 201 right after the DB write succeeds.
  //    Email is best-effort: failure is logged but never blocks the response.
  res.status(201).json({
    success: true,
  ...
  }).catch(() => { /* log failures must never crash the process */ });
}
